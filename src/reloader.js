...
class Reloader {
    ...
    runPluginsByOrder(path, options) {
        options.pluginOrder.some((pluginId) => {
            ...
            if (pluginId === "external") {
                return this.plugins.some((plugin) => plugin.reload && plugin.reload(path, options))
            }
    ...
    reloadStylesheet(path) {
        ...
        const match = pickBestMatch(path, links.concat(imported), (link) => pathFromUrl(this.linkHref(link)))
